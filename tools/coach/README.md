...
`flutter run --dart-define=COACH_URL=http://<LAN IP>:8787/coach --dart-define=COACH_TOKEN=...` with the
access token from `COACH_TOKEN` or `supabase/functions/.env`. Use a debug build (the default): release
...
- Claude runs with adaptive thinking at a per-route effort (`low` for questions and `ask`, `medium` for
  feedback and notes). Thinking counts against `max_tokens`, so each call gets 12,000 tokens of room on top of the reply.
